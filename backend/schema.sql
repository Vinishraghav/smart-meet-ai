-- SmartMeet AI Database Schema for Neon Postgres
...
CREATE TABLE IF NOT EXISTS meetings (
    id VARCHAR(255) PRIMARY KEY DEFAULT 'm' || encode(gen_random_bytes(16), 'hex'),
...
CREATE TABLE IF NOT EXISTS participants (
    id VARCHAR(255) PRIMARY KEY DEFAULT 'p' || encode(gen_random_bytes(16), 'hex'),
...
CREATE TABLE IF NOT EXISTS transcripts (
    id VARCHAR(255) PRIMARY KEY DEFAULT 't' || encode(gen_random_bytes(16), 'hex'),
    meeting_id VARCHAR(255) NOT NULL REFERENCES meetings(id) ON DELETE CASCADE,
